...
class Parser
    ...
    def initialize(file_path)
        ...
        File.readlines(file).each do |line|
            ...
            # joining results based on index
            if (line.split("|").include?(index.to_s)) 
    ...
    def get_result(result)
        return result if Float(result) rescue false
        return result.float if Integer(result) rescue false
    ...
    def get_comment(element)
        ...
        comment.join if comment.length > 0
    end
    ...
    def mapped_results
        ...
        @results_from_file.each do |element|
            ...
            laboratory_test_results << LaboratoryTestResult.new(result[:code], result[:result], result[:format], result[:comment])
        end
